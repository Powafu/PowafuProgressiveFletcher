...
public class ExTrade {
    ...
    public static String getTraderName() {
        ...
        if (traderNameComp != null && traderNameComp.isVisible() && traderNameComp.getText() != null) {
            String text = StringCommons.replaceColorTag(traderNameComp.getText());
            //Log.fine("name text: " + text);
    ...
    public static int getOfferPrice(boolean theirOffer) {
        ...
        if (priceComp != null && priceComp.isVisible()) {
            ...
            else if (priceText.contains("Lots")) {
                return 2147000000;
            }
            else {
    ...
    private static final InterfaceAddress SECOND_SCREEN_MY_ITEMS = new InterfaceAddress(() ->
            Interfaces.getComponent(SECOND_TRADE_WINDOW_PARENT_INDEX, SECOND_SCREEN_MY_ITEMS_INDEX));
    private static final InterfaceAddress SECOND_SCREEN_THEIR_ITEMS = new InterfaceAddress(() ->
            Interfaces.getComponent(SECOND_TRADE_WINDOW_PARENT_INDEX, SECOND_SCREEN_THEIR_ITEMS_INDEX));

    private static List<RSItemDefinition> getSecondTradeScreenDefinitions(boolean theirItems) {
        List<RSItemDefinition> itemDefinitions = new ArrayList<>();
        InterfaceComponent itemContainerComp = theirItems ? Interfaces.lookup(SECOND_SCREEN_MY_ITEMS) : Interfaces.lookup(SECOND_SCREEN_THEIR_ITEMS);
